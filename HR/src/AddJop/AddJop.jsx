...
const sendLocationIdToShipping = (selectedLocationId) => {
  ...
  async function onSubmit(values) {
    try {
      ...
      // console.log(data.errors., "Success");
...
console.log(locaionidd , "locaioniddlocaioniddlocaionidd");
// ...
useEffect(() => {
  ...
  let validationSchema = yup.object({
    user_name: yup.string().min(3, "min is 3").max(10, "max is 10").required("not required"),
    password: yup.string().required("required"),
    email:  yup.string().email().required("required"),
    phone: yup.string().matches(/[0-9]{5,13}/, "phone is not required").required(),
    location_id: yup.string().required(),
    address: yup.string().required("العنوان مطلوب"),
    company_name: yup.string().min(3, "min is 3").max(10, "max is 10").required("not required"),
  ...
  return <>
    ...
    <select
      ...
      {errorr ? <div className='alert alert-danger text-dark'>{errorr.logo}</div> : ""}
      {/* {errorr.email ? <div className='alert alert-danger text-dark'>{errorr.email}</div> : ""} */}
...
{formik.errors.logo && formik.touched.logo ? (
  <div className='alert alert-danger'>{formik.errors.logo}</div>
        ...
        <button disabled={(!formik.isValid && formik.dirty)} className='btn btn-add-card mt-4 w-100 border-radius p-8' type="submit">{t("register")} </button>
